...
| **BL-WEB-13** | Third taxonomy tier. The classification defines Category > Topic > Subject; Subject is modelled but unpopulated. Support the depth so it is visibly empty rather than silently absent. **Shipped**: the gap was already declared once in the STD collection's repairs list; now also stated directly on every topic bead's own facts ("Subject (3rd tier): not populated in this dataset"), visible where a reader is actually looking, not only in a separate report. | DONE | 2 | STD |
...
| **BL-WEB-15** | Mapping confidence as a first-class visual channel. 120 of 173 GDS mappings are inferred and 26 flagged for review; the web should encode that, not just report it. **Blocked, same class of issue as BL-WEB-53**: the 25/120/26 split exists only as an aggregate claim (this collection's own repairs entry, and the hand-authored bar chart on the Collections page) — `GDS_AREAS` carries no per-item confidence field, so there is no real basis for assigning individual mappings to OK/INFERRED/REVIEW without fabricating which ones. The one genuinely real per-item signal (the "[REVIEW] unmapped" area) is already flagged via `open`/`dormant` state. Needs the actual per-mapping classification from the source workbook before this can be done honestly. | P1 | 2 | GDS, ADR-W03 |
...
| **BL-WEB-33** | Quantified metadata drives the visual. impact and confidence (1–5) and severity should set bead weight and route prominence, so high-impact low-confidence findings surface automatically. **Blocked, same class of issue as BL-WEB-15/53**: the rendering path already exists (`pips('Impact', nd.impact)` in the detail rail, gated on `nd.impact \|\| nd.conf`) but no node anywhere in this codebase actually sets `impact`/`conf`/`severity` — confirmed by grep across every collection's data. The display mechanism is real and ready; there is no real scored data to drive bead weight or route prominence from. | P1 | 2 | KISIL §2 |
...
| **BL-WEB-41** | Provenance record per node — source URI, created-by role, version, lineage, review status. **Partial**: created-by role and review status added to the existing node-inspector Provenance quadrant — both a genuine, universally-true fact (everything here is AI-drafted, not owner-reviewed, per risk R-04), not per-node fabricated detail. Source URI and lineage were already covered (conversation link where real; partOf/derivedFrom edges in the same quadrant). Still missing: per-node version — no source collection carries real per-row version metadata to draw from. | P1 | 2 | MG-NFR-004 |
...
| **BL-WEB-51** | Performance budget. Declare and test a ceiling — first paint, interaction latency at N beads, memory. **Shipped**: four ceilings declared in `docs/programme-authority.html` Section 14 and tested automatically in `scripts/verify.mjs` every run (first paint, largest-collection switch, interaction latency, JS heap). Ceilings set after measuring actual behaviour, not guessed. | DONE | 2 | SELF |
...
| **BL-WEB-56** | Scheduled link revalidation. Live checking found 12 of 71 GDS product links now redirect and 3 are broken, while the workbook's own audit column still records them as verified. A verification stamp records when a link was checked, not whether it still resolves to the same place. **Stamp shipped**: the existing one-time check (25 July 2026, already reported on the Applications page) now also surfaces as an explicit "Link verification stamp" in the GDS collection's Provenance panel — checked count, reachable/redirected/broken, and the date, stated as a point-in-time fact, not a current-resolves-to claim. "Scheduled" (recurring, automatic) genuinely can't be done from a static single-file artefact with no server — there's nowhere for a schedule to run. | P1 | 1 | GDS, SELF |
